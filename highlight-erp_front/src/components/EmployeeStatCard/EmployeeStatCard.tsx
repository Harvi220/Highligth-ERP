...
const EmployeeStatCard = ({ employee }: EmployeeStatCardProps) => {
  ...
  const readDocuments = employee.documents.filter(doc => doc.status === 'read');
  const unreadDocuments = employee.documents.filter(doc => doc.status === 'assigned');
      ...
      <div className={styles.header} onClick={() => setIsExpanded(!isExpanded)}>
        <div className={styles.employeeInfo}>
          ...
          <div className={styles.details}>
            ...
            <div className={styles.position}>{employee.position?.name || 'Должность не указана'}</div>
          </div>
        </div>
            ...
            <ul className={styles.documentsList}>
              {employee.documents.map((doc) => (
                <li key={doc.id} className={styles.documentItem}>
                  <div className={`${styles.docIcon} ${doc.status === 'read' ? styles.docIconRead : styles.docIconUnread}`}>
                    {doc.status === 'read' ? (
