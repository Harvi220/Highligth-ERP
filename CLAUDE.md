...
3. **CreateDocumentPage** (`/admin/documents/create`)
   - Двухшаговая форма создания документа
   - Поля: название, описание, файл (PDF/DOCX/XLSX)
...
5. **CreateEmployeePage** (`/admin/employees/create`)
   - Двухшаговая форма создания сотрудника
...
2. **Очистка телефона**: Перед отправкой на сервер телефон очищается от нецифровых символов: `phone.replace(/\D/g, '')`
3. **Хранение пользователя**: Данные пользователя хранятся в `localStorage` под ключом `user_data`
4. **FormData**: Для загрузки файлов используется FormData с полями: `title`, `description`, `file`, `is_for_all_employees`
5. **Синхронизация документов**: При обновлении сотрудника отправляется полный массив ID документов (sync, не attach/detach)
6. **Защита от открепления**: Бэкенд запрещает открепление документов со статусом "read"